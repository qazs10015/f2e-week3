...
export class BusStatusDetailComponent implements OnInit {
  ...
  async ngOnInit() {
    this.search();
    this.deviceType = this.deviceDetectorService.getDeviceInfo()
  ...
  search() {
    ...
    forkJoin([
      ...
      this.cityBusService.getEstimatedTimeOfArrival(
        this.city,
        this.routeName,
        `PlateNumb ne '-1'`
      ),
        ...
        map((val: any[]) => {
          ...
          const bus = lstStops.map((stopInfo) => {
            ...
            return allStops.map((stop) => {
              const bus = lstBusN1EstimateTime.find(
                (n1) =>
                  n1.StopUID === stop.StopUID &&
                  n1.Direction === stopInfo.Direction
              );
              let result: any = {};
              let statusMsg = '';
              if (
                bus &&
                bus.StopUID === stop.StopUID &&
                bus.Direction === stopInfo.Direction
              ) {
      ...
      .subscribe((val: any[]) => {
        ...
        this.distinationStopName =
          this.lstStopData[this.lstStopData.length - 1].stopName;
      });
  }
